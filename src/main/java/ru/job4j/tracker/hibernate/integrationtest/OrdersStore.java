...
public class OrdersStore implements Service {
    ...
    public Order update(Order order) {
        try (Connection con = pool.getConnection();
             PreparedStatement pr = con.prepareStatement("UPDATE orders SET "
                     + "name = ?, "
                     + "description = ?,"
                     + " created = ? WHERE id = ?;", PreparedStatement.RETURN_GENERATED_KEYS)) {
            pr.setString(1, order.getName());
            pr.setString(2, order.getDescription());
            pr.setTimestamp(3, order.getCreated());
